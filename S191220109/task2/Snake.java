...
public class Snake {
    ...
    private void execute(String step, Line line) {
        ...
        int temp0 = Integer.parseInt(couple[0]), temp1 = Integer.parseInt(couple[1]);
        Monster monster0 = new Monster(0);
        Monster monster1 = new Monster(0);
        for (int i = 0; i < line.positions.length; ++i) {
            if (temp0 == line.get(i).rank())
                monster0 = line.get(i);
        }
        for (int i = 0; i < line.positions.length; ++i) {
            if (temp1 == line.get(i).rank())
                monster1 = line.get(i);
        }
        monster0.swapPosition(monster1);
    }
